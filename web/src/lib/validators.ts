...
export function isFilled(g: GlobalLayer, fieldId: string): boolean {
  switch (fieldId) {
    case "time":       return !!(g.season || g.time_of_day);
...
export function isShotFilled(s: Shot, fieldId: string): boolean {
  switch (fieldId) {
    ...
    case "lines":     return !!(s.lines?.text || s.lines?.audio_url);
    case "mono":      return !!(s.mono?.text || s.mono?.audio_url);
    case "narration": return !!(s.narration?.text || s.narration?.audio_url);
...
export function computeValidation(p: Project): ValidationResult {
  ...
  p.shots.forEach((s, i) => {
    if (!isShotFilled(s, "action")) {
      missing.push(`分镜${String(i + 1).padStart(2, "0")} · 07 动作`);
    }
  });
